...
function findFirstSingleChar(str) {
  ...
  str.split('').forEach((char) => {
    if (charsArr.find((el) => Object.keys(el).includes(char))) {
      const found = charsArr.find((el) => Object.keys(el).includes(char));
      found[char] += 1;
    } else {
      charsArr.push({ [char]: 1 });
    }
  });
  const res = charsArr.find((el) => Object.values(el).includes(1));
  return res ? Object.keys(res)[0] : null;
}
 ...
 *   7  9  9  2  7  3  9  8  7  1  3
    14  9 18  2 14  3 18  8  14 1  6
     5  9  9  2  5  3  9  8  5  1  6
 * 5+9+9+9+5+3+9+8+5+1+6=39+15+8+3+1+6=40+29+3=72
  ...
  return (
    String(ccn)
      ...
      .reduce((acc, item, i, arr) => {
        let num = (arr.length - 1 - i) % 2 === 1 ? item * 2 : item;
        if (num > 9) num -= 9;
        return acc + num;
      }, 0) %
      10 ===
...
function getDigitalRoot(num) {
  const numArr = String(num)
    .split('')
    .map((el) => Number(el));
  const sum = numArr.reduce((acc, item) => acc + item, 0);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (values.includes(str[i])) {
      const index = values.indexOf(str[i]);
      if (temp[temp.length - 1] === keys[index]) {
        temp.pop();
      } else {
        return false;
      }
  ...
  for (let i = 0; i < pathes[0].length; i += 1) {
    for (let j = 1; j < pathes.length; j += 1) {
      if (pathes[0][i] !== pathes[j][i]) {
        return res.length <= 1
          ? res
          : `${res.split('/').slice(0, -1).join('/')}${res.length > 2 ? '/' : ''}`;
      }
    }
    res += pathes[0][i];
  }
